...
(function(global) {
  ...
  var util = global.util;  // Exposed by utils.js.
  ...
  var sounds = {
    died: {
      audio: new Audio('sounds/dead.wav'),
      mediaElementSource: null  // Created in init.
    },
    won: {
      audio: new Audio('sounds/succes.wav'),
      mediaElementSource: null  // Created in init.
    },
    collectablePickup: {
      audio: new Audio('sounds/collect.wav'),
      mediaElementSource: null  // Created in init.
    }
  };
  ...
  var Collectable = function() {
    ...
    this.render = function() {
      ctx.drawImage(Resources.get(this.sprite), this.x, this.y, 45, 53);
    };
  ...
  function drawLives() {
    ...
    for (var i = 0, len = gameState.lives; i < len; i++) {
      ctx.drawImage(Resources.get('images/Heart.png'), x, 10, 30, 42);
      x += 30;
    }
  }
  ...
  function drawScore() {
    ...
    ctx.fillText('Points: ' + gameState.score, 101 * 5, 45);
    ctx.restore();
  }
  ...
  function drawScoresMenu() {
    ...
    ctx.strokeStyle = '#000';
    ctx.fillStyle = '#393';
    ctx.fillRect(0, 6, canvas.width, canvas.height - 5);
    ...
    ctx.textBaseline = 'alphabetic';  // top, hanging, middle, alphabetic, ideographic, bottom
    ctx.fillText('Hall of fame', (canvas.width - 2) / 2, 60);
  ...
  function drawStartMenu() {
    ...
    ctx.fillRect(0, 6, canvas.width, canvas.height - 5);
    ctx.strokeRect(1, 6, canvas.width - 2, canvas.height - 6);
    ...
    ctx.textAlign = 'center';  // start, end, left, right, center
    ctx.textBaseline = 'alphabetic';  // top, hanging, middle, alphabetic, ideographic, bottom
    ctx.fillText('Clone Frogger', (canvas.width - 2) / 2, 86);
    ...
    ctx.textAlign = 'center';  // start, end, left, right, center
    ctx.textBaseline = 'middle';  // top, hanging, middle, alphabetic, ideographic, bottom
    ctx.fillText('Start', (canvas.width - 2) / 2, canvas.height - 100);
